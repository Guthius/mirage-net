...
internal sealed class WinCreateAccount : WinBase
{
    public WinCreateAccount(Window window, ISceneMainMenu menu) : base(window)
    {
        var textBoxAccountName = Get<TextBox>("AccountNameTextBox");
        var textBoxPassword = Get<TextBox>("PasswordTextBox");

        Get<Button>("CreateButton").Click += (_, _) => menu.CreateAccount(textBoxAccountName.Text, textBoxPassword.Text);
